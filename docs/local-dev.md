...
```bash
cd ~/Projects/yield-curve-visualiser
...
`yield_observations` / `spread_observations`). `VITE_API_URL` and the
Railway fallback are gone; if `.env.local` is missing or empty the app
will fail to fetch data outright rather than silently falling back to
anything.

## Credentials
- FRED API key: stored in `yield-curve-visualiser/.env` as `FRED_API_KEY`
  (used by the per-country fetch modules and `seed_supabase.py`)
- Supabase service credentials: stored in `yield-curve-visualiser/.env`
  as `SUPABASE_URL` / `SUPABASE_KEY` — required to run `seed_supabase.py`,
  which populates the Supabase tables the deployed frontend reads from

## VSCode interpreter
Set to: `~/Projects/yield-curve-visualiser/.venv/bin/python`

...
- The 1Y/2Y/4Y/10Y/Max buttons render in Dashboard.jsx, NOT inside
  ...
  calls setYears and never reads it, so its closure comment and
  eslint-disable remain correct. Dashboard gates the pills on
  SPREAD_COUNTRIES.some(selected) because SpreadChart early-returns its
  empty state before the plot — without that gate the pills would float
  over the "pick a supported country" notice.
  (2026-07-31 they sat on the chart card's tab rail; since the 2026-08-29
  light redesign the tabs moved to the app header, so the pills now sit on
  the chart card's own title row, `.chart-card-head`. Same state, same
  gate, different row.)

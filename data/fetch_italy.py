# data/fetch_italy.py
#
# Fetches Italian government bond yields from the ECB's data API.
#
# Public contract (same shape as get_us_yields / get_uk_yields / get_germany_yields):
#
#   get_italy_yields(date: str) -> (actual_date: str | None, yields: pd.Series)
#
...
# like B.IT.EUR.4F.G_N_A.SV_C_YM.SR_10Y returns a clean JSON 404 "No
...
# France and Spain) only produces "10Y". classify_curve() needs 2+
# maturities, so Italy's curve shows up as "not enough maturities to
...
ITALY_SERIES = {
    "10Y": "M.IT.L.L40.CI.0000.EUR.N.Z",
}
...
def get_italy_yields(date):
    """
    Fetch Italian government bond yields for every maturity in
    ITALY_SERIES, closest to `date`.
    ...
    for maturity, series_key in ITALY_SERIES.items():
        try:
